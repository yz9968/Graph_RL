...
class MultiAgentEnv_GRL(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        return self.get_obs(next_obs), next_adj, reward, done_signals, info
...
class MultiAgentEnv_ppo_cnn(MultiAgentEnv):
    def __init__(self, world, reset_callback=None, reward_callback=None,
        ...
        self.agent_num = len(self.agents)
        self.args = args
        self.args.obs_shape = [1296 for _ in range(self.agent_num)]
        # scenario callbacks
        self.reset_callback = reset_callback
        self.reward_callback = reward_callback
        self.observation_callback = observation_callback
    ...
    def create_init_img(self, obs):
        """
        :param obs:
        :return:
        """
        boundary = self.world.boundary[1]
        img = np.zeros([80, 80], np.uint8) + 255
        obs = np.rint(obs * 40 / boundary)
        for i, agent in enumerate(self.agents):
            pos = (int(obs[i][0]), int(obs[i][1]))
            img[pos[0]][pos[1]] = 0
        cv2.imwrite('img.png', img)
        return img.reshape((1, 1, 80, 80))

    def create_img(self, obs, next_obs):
        boundary = self.world.boundary[1]
        img = np.zeros([80, 80], np.uint8) + 255
        obs = np.rint(obs * 40 / boundary)
        next_obs = np.rint(next_obs * 40 / boundary)
        for i, agent in enumerate(self.agents):
            pos1 = (int(obs[i][0]), int(obs[i][1]))
            pos2 = (int(next_obs[i][0]), int(next_obs[i][1]))
            img[pos1[0]][pos1[1]] = 0
            img[pos2[0]][pos2[1]] = 0
    ...
    def reset(self):
        ...
        self.reset_callback(self.world)
        for i, agent in enumerate(self.agents):
            policy = PPO_CNN(self.args, agent.agent_id)
            agent.set_policy(policy)
        self.agents = self.world.agents
        self.agent_num = len(self.agents)
        self.simulation_done = False
        ...
        self.states.append(obs)
        obs = self.get_obs(obs)
        img_state = self.create_init_img(obs)
        return img_state
    ...
    def step(self, actions):
        ...
        next_img_state = self.create_img(obs, next_obs)

        return next_img_state, reward, done_signals, info
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            start = mlines.Line2D(start_x, start_y, color=start_color, marker='*', linestyle='None', markersize=15,
                                  label='Goal')
            conflict_num = plt.text(-boundary, boundary, 'conflict_num %d' % (self.conflict_num_episode), color='red', fontsize=18)
            label = plt.text(boundary / 2, boundary, 'ppo_cnn', color='blue', fontsize=18)
            ax.add_artist(goal)
            ax.add_artist(start)
            ax.add_artist(conflict_num)
            ax.add_artist(label)
...
# environment for all agents in the multiagent world of ppo
class MultiAgentEnv_ppo_lstm(MultiAgentEnv):
    def __init__(self, world, reset_callback=None, reward_callback=None,
                 observation_callback=None, info_callback=None,
                 done_callback=None, args = None, shared_viewer=True):
        super(MultiAgentEnv, self).__init__()
    ...
    def reset(self):
        ...
        self.reset_callback(self.world)
        for i, agent in enumerate(self.agents):
            policy = PPO_LSTM(self.args, agent.agent_id)
            agent.set_policy(policy)
            agent.route_len = 0
        self.agents = self.world.agents
        self.agent_num = len(self.agents)
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            start = mlines.Line2D(start_x, start_y, color=start_color, marker='*', linestyle='None', markersize=15,
                                  label='Goal')
            conflict_num = plt.text(-boundary, boundary, 'conflict_num %d' % (self.conflict_num_episode), color='red', fontsize=18)
            label = plt.text(boundary, boundary, 'ppo_lstm', color='blue', fontsize=18)
            ax.add_artist(goal)
            ax.add_artist(start)
            ax.add_artist(conflict_num)
            ax.add_artist(label)

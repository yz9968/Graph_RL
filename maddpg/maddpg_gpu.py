...
class MADDPG:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        self.actor_model_name = '/15_actor_params.pkl'
        self.critic_model_name = '/15_critic_params.pkl'
        # 加载模型
    ...
    def train(self, transitions, other_agents):
        ...
        with torch.no_grad():
            ...
            for agent_id in range(self.args.n_agents):
                if agent_id == self.agent_id:
                    u_next.append(self.actor_target_network(o_next[agent_id].to(self.device)))
                else:
                    # 因为传入的other_agents要比总数少一个，可能中间某个agent是当前agent，不能遍历去选择动作
                    u_next.append(other_agents[index].policy.actor_target_network(o_next[agent_id].to(self.device)))
                    index += 1
        ...
        u[self.agent_id] = self.actor_network(o[self.agent_id].cuda())
